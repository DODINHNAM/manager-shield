...
register_activation_hook(__FILE__, 'mecom_gateway_stripe_install');

require_once(plugin_dir_path(__FILE__) . 'utils.php');

$mecomStripeSettings = get_option('woocommerce_mecom_stripe_settings');
if (isset($mecomStripeSettings['sync_tracking_automatic']) && $mecomStripeSettings['sync_tracking_automatic'] === 'yes' && !wp_next_scheduled('mecom_gateway_stripe_cron_auto_sync')) {
    wp_schedule_event(time(), 'hourly', 'mecom_gateway_stripe_cron_auto_sync');
}
add_action('mecom_gateway_stripe_cron_auto_sync', 'mecom_gateway_stripe_cron_auto_sync_process');

add_action('update_option_woocommerce_mecom_stripe_settings', function ($_old, $new) {
    if (($new['sync_tracking_automatic'] ?? '') !== 'yes') {
        $timestamp = wp_next_scheduled('mecom_gateway_stripe_cron_auto_sync');
        if ($timestamp) {
            wp_unschedule_event($timestamp, 'mecom_gateway_stripe_cron_auto_sync');
        }
    }
}, 10, 2);
...
function mecom_gateway_stripe_cron_auto_sync_process()
{
...
//Cron
add_filter('cron_schedules', 'mecom_add_stripe_cron_interval');
if (!wp_next_scheduled('mecom_gateway_stripe_rotation')) {
    wp_schedule_event(time(), 'one_minute', 'mecom_gateway_stripe_rotation');
}

add_action('mecom_gateway_stripe_rotation', 'mecom_stripe_rotation_checker');
if (!wp_next_scheduled('mecom_gateway_stripe_daily')) {
    try {
    ...
    wp_schedule_event($firstRun, 'daily', 'mecom_gateway_stripe_daily');
}
add_action('mecom_gateway_stripe_daily', 'mecom_gateway_stripe_daily_process');

function mecom_gateway_stripe_daily_process()
{
    // Reset paid amount
    $rotationMethod = get_option(OPT_MECOM_STRIPE_ROTATION_METHOD, MECOM_STRIPE_BY_TIME);
    if ($rotationMethod === MECOM_STRIPE_BY_AMOUNT) {
        resetPaidAmountStripe();
    }
}

add_action('plugins_loaded', 'mecom_add_gateway_stripe_init');
...
function mecom_add_stripe_cron_interval($schedules)
{
...
function mecom_gateway_stripe_install()
{
//    delete_option('mecom_gateway_stripe_version');
//    add_option('mecom_gateway_stripe_version', uniqid());
}
...
function action_woocommerce_admin_order_totals_after_total_stripe($order_get_id)
{
    ...
    if ($wc_order->get_payment_method() !== 'mecom_stripe') {
        return;
    }
...
function mecom_stripe_rotation_checker()
{

    $rotationMethod = get_option(OPT_MECOM_STRIPE_ROTATION_METHOD, MECOM_STRIPE_BY_TIME);
    if ($rotationMethod != MECOM_STRIPE_BY_TIME) {
        return;
    }
    ...
    $lastActivatedTimestamp = get_option(OPT_MECOM_STRIPE_CURRENT_ROTATION_VALUE, 0);
        ...
        if ($minutes > floatval($activatedProxy["timestamp"])) {
            ...
            for ($i = 0; $i < count($proxies); $i++) {
                ...
                if ($proxy["id"] == $activatedProxy["id"]) {
                    ...
                    update_option(Opt_Mecom_Stripe_Activated_Proxy, $needToActivateProxy, true);
                    update_option(OPT_MECOM_STRIPE_CURRENT_ROTATION_VALUE, time(), true);
                    logStripeRotation(MECOM_STRIPE_BY_TIME, $needToActivateProxy, "Auto");
                    $hasDeletedActivateProxy = false;
                    break;
                }
            }
    ...
    if (empty($activatedProxy) || $hasDeletedActivateProxy) {
        update_option(Opt_Mecom_Stripe_Activated_Proxy, $proxies[0], true);
        update_option(OPT_MECOM_STRIPE_CURRENT_ROTATION_VALUE, time(), true);
        logStripeRotation(MECOM_STRIPE_BY_TIME, $proxies[0], "Auto");
    }
}

function handle_route()
{
    if (isset($_GET['mecom_stripe_return_result']) && isset($_GET['order_id'])) {
        $order = wc_get_order($_GET['order_id']);
        csStripeDebugLog($_GET, 'mecom_stripe_return_result info [1]');
        if (!$order) {
            csStripeErrorLog('mecom_stripe_return_result ERROR [1]');
        ...
        $response = wp_remote_post($activatedProxy['url'] . '?' . csStripeBuildQuery([
                'mecom-stripe-pe-v2-confirm-payment' => uniqid(),
        ...
        $paymentStripeIntent = get_option('woocommerce_mecom_stripe_settings')['intent'];
        ...
        if ($body->status === 'success') {
            ...
            if ($paymentStripeIntent === OPT_MECOM_STRIPE_INTENT_AUTHORIZE) {
                $order->add_order_note(sprintf(__('Stripe authorize by proxy %s, (Payment Intent ID: %s)', 'mecom'), $activatedProxy['url'], $paymentIntent->id));
                $order->update_status('on-hold', 'Payment can be captured.');
                $order->update_meta_data( METAKEY_MECOM_STRIPE_INTENT_AUTHORIZED, 'true');
            } else {
                if ($paymentIntent->status === 'processing') {
                    ...
                    $order->add_order_note(sprintf(__('Stripe processing by proxy %s', 'mecom'), $activatedProxy['url']), 0, false);
                    $order->add_order_note(sprintf(__('Stripe Checkout processing (Payment Intent ID: %s)', 'mecom'), $paymentIntent->id));
                } else {
                    $order->update_meta_data( METAKEY_MECOM_STRIPE_CAPTURED, 'true');
                    if (!$order->is_paid()) {
                        $order->payment_complete();
                        $order->add_order_note(sprintf(__('Stripe charged by proxy %s', 'mecom'), $activatedProxy['url']), 0, false);
                        $order->add_order_note(sprintf(__('Stripe Checkout charge complete (Payment Intent ID: %s)', 'mecom'), $paymentIntent->id));
                    } else {
                        csStripeErrorLog('mecom_stripe_return_result already paid!');
                    }
                }
            }
    ...
    if (isset($_GET['cs-stripe-notify-payment-status']) && isset($_GET['order_id'])) {
        ...
        $response = wp_remote_get($activatedProxy['url'] . '?' . csStripeBuildQuery([
                'mecom-stripe-pe-v2-get-payment-intent' => uniqid(),
                'payment_intent_id' => csStripeGetTransactionId($order),
                'merchant_site' => get_home_url(),
            ]), [
            'sslverify' => csStripeGetSSLVerifyStatus(),
        ...
        if ($body->status === 'success' && isset($body->payment_intent->status)) {
            if ($body->payment_intent->status === 'succeeded') {
                if (!$order->is_paid()) {
                    $order->payment_complete();
                    $order->add_order_note(sprintf(__('Stripe charged by proxy %s', 'mecom'), $activatedProxy['url']), 0, false);
                    $order->add_order_note(sprintf(__('Stripe Checkout charge complete (Payment Intent ID: %s)', 'mecom'), $body->payment_intent->id));
                ...
                wp_remote_get($activatedProxy['url'] . '?' . csStripeBuildQuery([
                        'mecom-stripe-pe-v2-add-payment-history' => uniqid(),
                        'payment_intent_id' => csStripeGetTransactionId($order),
                        'merchant_site' => get_home_url(),
                    ]), [
                    'sslverify' => csStripeGetSSLVerifyStatus(), 
            ...
            wp_remote_post($activatedProxy['url'] . '?' . csStripeBuildQuery([
                'mecom-stripe-pe-v2-add-order-detail' => uniqid(),
    ...
    if (isset($_GET['handle_scs_notice_success']) && isset($_GET['oid']) && isset($_GET['ssi'])) {
        ...
        if ($body->status === 'success') {
            ...
            $order->update_meta_data( METAKEY_CS_STRIPE_RAND_ORDER_ID, $paymentIntent->metadata->order_id ?? null);
            if ($paymentStripeIntent === OPT_MECOM_STRIPE_INTENT_AUTHORIZE) {
                $order->add_order_note(sprintf(__('Stripe authorize by proxy %s, (Checkout Session ID: %s, Payment Intent ID: %s)', 'mecom'), $activatedProxy['url'], $_GET['ssi'], $paymentIntent->id));
                $order->update_status('on-hold', 'Payment can be captured.');
                $order->update_meta_data( METAKEY_MECOM_STRIPE_INTENT_AUTHORIZED, 'true');
            } else {
                if ($paymentIntent->status === 'processing') {
                    ...
                    $order->update_status('on-hold', 'Payment processing.');
                    $order->add_order_note(sprintf(__('Stripe processing by proxy %s', 'mecom'), $activatedProxy['url']), 0, false);
                    $order->add_order_note(sprintf(__('Stripe Checkout processing (Checkout Session ID: %s, Payment Intent ID: %s)', 'mecom'), $_GET['ssi'], $paymentIntent->id));
                } else {
                    $order->update_meta_data( METAKEY_MECOM_STRIPE_CAPTURED, 'true');
                    $order->payment_complete();
                    $order->add_order_note(sprintf(__('Stripe charged by proxy %s', 'mecom'), $activatedProxy['url']), 0, false);
                    $order->add_order_note(sprintf(__('Stripe Checkout charge complete (Checkout Session ID: %s, Payment Intent ID: %s)', 'mecom'), $_GET['ssi'], $paymentIntent->id));
                }
            }
            $order->reduce_order_stock();
            ...
            $order->update_status('failed');
            $err = $body->err;
            $order->add_order_note(sprintf(__('Stripe charged ERROR by proxy %s, ERROR message: %s, Checkout Session ID: %s', 'mecom'),
                $activatedProxy['url'],
                is_string($err) ? $err : $err->message,
                $_GET['ssi']
            ));
    ...
    if (isset($_GET['handle_scs_notice_failed']) && isset($_GET['h']) && isset($_GET['oid']) && isset($_GET['ssi'])) {
        ...
        $order->update_status('failed');
        $order->add_order_note(sprintf(__('Stripe charged ERROR by proxy %s, ERROR message: %s, Checkout Session ID: %s, Payment Intent ID: %s', 'mecom'),
            $activatedProxy['url'],
...
function mecom_add_gateway_stripe_init()
{

    if (is_admin()) {
        require_once("m-ecom-stripe-options.php");
    }

    if (!class_exists('WC_MEcom_Stripe')) :

        class WC_MEcom_Stripe
        {
            ...
            private function init()
            {
                ...
                add_filter('woocommerce_order_actions', [$this, 'add_mecom_stripe_order_actions'], 10, 1);
                add_action('woocommerce_order_action_mecom_stripe_capture_authorization_order', [$this, 'mecom_stripe_capture_authorization_order']);
                add_action('woocommerce_order_action_mecom_stripe_cancel_authorization_order', [$this, 'mecom_stripe_cancel_authorization_order']);
            }
            ...
            public function check_cs_stripe_payment_gateways($gateways)
            {
                ...
                if ($isEnableEndpointMode) {
                    ...
                    if (!$shieldUrl) {
                        unset($gateways['mecom_stripe']);
                    }
                } else {
                    $rotationMethod = get_option(OPT_MECOM_STRIPE_ROTATION_METHOD, MECOM_STRIPE_BY_TIME);
                    global $woocommerce;
                    $activeProxy = get_option(Opt_Mecom_Stripe_Activated_Proxy, null);
                    ...
                    if (empty(get_option(Opt_Mecom_Stripe_Proxies, []))) {
                        unset($gateways['mecom_stripe']);
                    }
                }
                return $gateways;
            ...
            public function add_mecom_stripe_order_actions($order_actions)
            {
                global $theorder;
                ...
                $order_actions['mecom_stripe_capture_authorization_order'] = 'Capture authorized Stripe payment';
                $order_actions['mecom_stripe_cancel_authorization_order'] = 'Void authorized Stripe payment';
            ...
            {
                return $wc_order->get_meta(METAKEY_MECOM_STRIPE_INTENT_AUTHORIZED) === 'true';
            }

            function mecom_stripe_capture_authorization_order(WC_Order $wc_order)
            {
                ...
                $capturePaymentUrl = $proxyUrl . "?mecom-stripe-pe-v2-capture-payment=1&" . csStripeBuildQuery($params);
                ...
                $wc_order->add_order_note(sprintf(__('Stripe Capture complete (Payment Intent ID: %s)', 'mecom'), $paymentIntent->id));
                $wc_order->update_meta_data(METAKEY_MECOM_STRIPE_INTENT_AUTHORIZED, 'false');
                $wc_order->update_meta_data(METAKEY_MECOM_STRIPE_CAPTURED, 'true');
                $wc_order->save();
                $wc_order->payment_complete();
                return true;
            }

            function mecom_stripe_cancel_authorization_order(WC_Order $wc_order)
            {
                if ($wc_order->get_status() == 'cancelled') {
                    return false;
                }
                ...
                $params["merchant_site"] = get_home_url();
                $params["payment_intent_id"] = $paymentIntentId;
                $cancelPaymentUrl = $proxyUrl . "?mecom-stripe-pe-v2-cancel-payment=1&" . csStripeBuildQuery($params);
                ...
                $wc_order->update_meta_data(METAKEY_STRIPE_SYNC_TRACKING_INFO, OPT_CS_STRIPE_NOT_SYNCED);
                updateFeeNetOrderStripe($data->charge, $wc_order);
                $wc_order->add_order_note(sprintf(__('Stripe Void complete (Payment Intent ID: %s)', 'mecom'), $paymentIntent->id));
                $wc_order->update_meta_data(METAKEY_MECOM_STRIPE_INTENT_AUTHORIZED, 'false');
                $wc_order->update_meta_data(METAKEY_MECOM_STRIPE_CAPTURED, 'true');
                $wc_order->save();
            ...
            {
                $gateways[] = 'WC_MEcom_Gateway_Stripe'; // your class name is here
                return $gateways;
            }
            ...
            public function add_settings_link($links)
            {
                $settings = array(
                    'settings' => sprintf(
                        '<a href="%s">%s</a>',
                        admin_url('admin.php?page=wc-settings&tab=checkout&section=mecom_stripe'),
                        'Settings'
                    )
                );
        ...
        WC_MEcom_Stripe::get_instance();

        if (!class_exists('WC_Payment_Gateway')) {
            return;
        }

        class WC_MEcom_Gateway_Stripe extends WC_Payment_Gateway
        {
            /**
            ...
            public static $mecom_stripe_is_inited = false;
            ...
            public function __construct()
            {
                $this->id = 'mecom_stripe'; // payment gateway plugin ID
                ...
                if (!self::$mecom_stripe_is_inited) {
                    self::$mecom_stripe_is_inited = true;
                    ...
                    if (isset($_GET['pay_for_order'])) {
                        add_action('woocommerce_pay_order_after_submit', [$this, 'mecom_stripe_add_button_hosted_checkout']);
                    } else {
                        add_action('woocommerce_review_order_after_payment', [$this, 'mecom_stripe_add_button_hosted_checkout']);
                    }
                }
            ...
            public function init_form_fields()
            {
                $this->form_fields = array(
                    ...
                    'intent' => [
                        ...
                        'options' => [
                            OPT_MECOM_STRIPE_INTENT_CAPTURE => 'Capture',
                            OPT_MECOM_STRIPE_INTENT_AUTHORIZE => 'Authorize',
                        ],
                    ],
                    'card_icons' => array(
                        'type' => 'multiselect',
                    ...
                    'payment_mode' => [
                        ...
                        'class' => [],
                        'input_class' => ['wc-enhanced-select'],
                        'default' => MECOM_STRIPE_PAYMENT_MODE_HOSTED,
...
<br/><b>- Card Form (not recommended)</b>: Takes credit card details directly on the checkout page and does not support Apple Pay or Google Pay.',
                        'options' => [
                            MECOM_STRIPE_PAYMENT_MODE_HOSTED => 'Stripe Checkout (highly recommended)',
                            MECOM_STRIPE_PAYMENT_MODE_EMBEDDED => 'Card Form',
                        ],
                    ],
...
*/',
            ...
            public function generate_config_proxies_button_html($key, $value)
            {
                ...
                <tr valign="top">
                    <td colspan="2" class="forminp forminp-<?php echo sanitize_title($value['type']) ?>">
                        <a href="<?php echo admin_url('admin.php?page=mecom-gateway-stripe'); ?>"
            ...
            public function get_icon()
            {
                ...
                if (is_array($icons)) {
                    foreach ($icons as $index => $icon) {
                        if ($index > 3) break;
                        $icons_str = '<img class="mecom-stripe-payment-icon" src="' . cs_stripe_icon_src('/assets/images/icons/' . $icon . '.svg', __FILE__, OPT_MECOM_STRIPE_VERSION) . '" style="float: right; border-radius: 2px; max-height: 25px;padding-top: 2px; margin-right: 4px"/>' . $icons_str;
                    }
                    if (count($icons) > 4) {
                        $icons_str = '<img class="mecom-stripe-payment-icon" src="' . cs_stripe_icon_src('/assets/images/icons/' . (count($icons) - 4) . '.svg', __FILE__, OPT_MECOM_STRIPE_VERSION) . '" style="float: right; border-radius: 2px; max-height: 25px;padding-top: 2px; margin-right: 4px"/>' . $icons_str;
                    }
                }
            ...
            public function admin_scripts()
            {
                ...
                wp_enqueue_script( 'woocommerce_stripe_admin', plugins_url('assets/js/payment_settings.js', __FILE__), array(), OPT_MECOM_STRIPE_VERSION, true );
            }

            function mecom_stripe_add_button_hosted_checkout()
            {
                if (is_checkout()) {
                    $gateways = WC()->payment_gateways->get_available_payment_gateways();
                    if (isset($gateways['mecom_stripe']->enabled) && $gateways['mecom_stripe']->enabled == 'yes') {
                        $nextProxyUrl = WC()->session->get('mecom-stripe-proxy-active-url');
                        if ($nextProxyUrl && $this->get_option('payment_mode') === MECOM_STRIPE_PAYMENT_MODE_HOSTED) {
            ...
            public function payment_fields()
            {
                ...
                if (isCsStripeEnableEndpointMode()) {
                    findAndSetNextProxy();
                    $nextProxy = ['id' => WC()->session->get('mecom-stripe-proxy-active-id'), 'url' => WC()->session->get('mecom-stripe-proxy-active-url')];
                } else {
                ...
                if (!isset($nextProxy['url'])) {
                    ...
                    <?php
                } else if ($this->get_option('payment_mode') === MECOM_STRIPE_PAYMENT_MODE_EMBEDDED) {
                    ...
                    if (isset($_GET['pay_for_order']) && get_query_var('order-pay')) {
                        mecom_stripe_generate_input_order();
                    }
                    $params = [
                    ...
                    ?>
                    <input style="display:none;" name="mecom-stripe-payment-method-id"/>
                    ...
                    <?php
                } else if ($this->get_option('payment_mode') === MECOM_STRIPE_PAYMENT_MODE_HOSTED) {
                    if (isset($_GET['pay_for_order']) && get_query_var('order-pay')) {
                        mecom_stripe_generate_input_order();
                    }
            ...
            public function payment_scripts()
            {
                ...
                wp_register_style('mecom_stripe_styles', plugins_url('assets/css/styles.css', __FILE__), [], OPT_MECOM_STRIPE_VERSION);
                wp_enqueue_style('mecom_stripe_styles');

                wp_register_script('mecom_stripe_js', plugins_url('assets/js/checkout_hook.js', __FILE__), array('jquery'), OPT_MECOM_STRIPE_VERSION, true);
                wp_enqueue_script('mecom_stripe_js');
                wp_localize_script('mecom_stripe_js', 'ajax_object', [
            ...
            {
                if ($this->get_option('payment_mode') === MECOM_STRIPE_PAYMENT_MODE_HOSTED) {
                ...
                $activeProxyId = WC()->session->get('mecom-stripe-proxy-active-id');
                $activeProxyUrl = WC()->session->get('mecom-stripe-proxy-active-url');
                $activatedProxy = [
                    'id' => $activeProxyId,
                    'url' => $activeProxyUrl,
                ];
                ...
                $order->add_order_note(sprintf(__('Starting checkout with Stripe proxy %s', 'mecom'), $activatedProxy['url']), 0, false);
                ...
                if (!empty(csStripeGetTransactionId($order))) {
                    ...
                    if ($proxyProcessingUrl) {
                        if (WC()->session->get('mecom-stripe-proxy-active-url') == $proxyProcessingUrl) {
                ...
                $response = wp_remote_post($activatedProxy['url'] . '?' . csStripeBuildQuery([
                        'mecom-stripe-pe-v2-make-payment' => uniqid(),
                        'capture_method' => $paymentStripeIntent === OPT_MECOM_STRIPE_INTENT_AUTHORIZE ? 'manual' : 'automatic',
                        'payment_intent' => $paymentIntentIdRequest,
                        'payment_method_id' => $_POST['mecom-stripe-payment-method-id'],
                ...
                $order->update_meta_data( METAKEY_STRIPE_PROCESSING_ORDER_KEY, WC()->session->get('mecom-stripe-processing-order-key'));
                $order->update_meta_data( METAKEY_STRIPE_PROXY_URL, $activatedProxy['url']);
                $order->update_meta_data( '_shield_payment_method', 'stripe');
                $order->update_meta_data( '_shield_payment_url', $activatedProxy['url']);
                $order->update_meta_data( METAKEY_STRIPE_PROXY_ID, $activatedProxy['id']);
                ...
                if ($body->status === 'success') {
                    ...
                    if (isset($body->payment_intent)) {
                        ...
                        $order->update_meta_data( METAKEY_CS_STRIPE_RAND_ORDER_ID, $body->payment_intent->metadata->order_id ?? null);
                        $order->save_meta_data();
                        $order->add_order_note(sprintf(__('Stripe confirm Payment Intent by proxy %s, Payment Intent ID: %s', 'mecom'),
                            $activatedProxy['url'],
                            $paymentIntentId
                        ));
                    }
            ...
            public function process_hosted_payment($order_id)
            {
                ...
                $response = wp_remote_get($activatedProxy['url'] . '?' . csStripeBuildQuery([
                        'mecom-stripe-hosted-make-session' => uniqid(),
                        "lang" => csGetCurrentLanguage(),
                        'capture_method' => $paymentStripeIntent === OPT_MECOM_STRIPE_INTENT_AUTHORIZE ? 'manual' : 'automatic',
                        'order_id' => $order->get_id(),
                ...
                if ($body->status === 'success') {
                    ...
                    $order->add_order_note(sprintf(__('Start redirect to checkout %s, Session ID: %s', 'mecom'),
                        $activatedProxy['url'],
                        $paymentSession->id
                    ));
                    ...
                    if ($body->code === 'domain_whitelist_not_allow') {
                        ...
                        $err = $body->err;
                        $order->add_order_note(sprintf(__('Stripe Create Payment Session ERROR by proxy %s, ERROR message: %s', 'mecom'),
                            $activatedProxy['url'],
                            is_string($err) ? $err : $err->message
                        ));
                    }
            ...
            public function process_refund($order_id, $amount = null, $reason = '')
            {
                ...
                if (0 == $amount || null == $amount) {
                    return new WP_Error('stripe_refund_error', __('Refund Error: You need to specify a refund amount.', 'mecom-stripe-gateway'));
                }

                try {
                    $result = $this->refund_order($order, $order_id, $amount, "", $reason);
                    $charge = $result['charge_obj'];
                    $order->add_order_note(sprintf(__('Stripe refund completed; transaction ID = %s', 'mecom-stripe-gateway'), csStripeGetTransactionId($order)));
            ...
            private function refund_order($order, $order_id, $amount, $refundType, $reason)
            {
                ...
                $url = $proxyUrl . "?" . csStripeBuildQuery([
                        'mecom-stripe-pe-v2-refund' => uniqid(),
                        'order_id' => $order_id,
                        'transaction_id' => csStripeGetTransactionId($order),
                ...
                if (is_wp_error($request)) {
                    ...
                    $order->add_order_note(sprintf(__('Failed refund by Stripe! Debug proxy %s', 'mecom-stripe-gateway'), $url));
        ...
        WC_MEcom_Gateway_Stripe::get_instance();
    endif;
}

register_deactivation_hook(__FILE__, 'cs_stripe_plugin_deactivation');
add_action('woocommerce_update_option', function ($event) {
    if($event['id'] === 'woocommerce_mecom_stripe_settings') {
        wp_clear_scheduled_hook( 'mecom_gateway_stripe_cron_auto_sync' );
    }
});
...
{
    wp_clear_scheduled_hook( 'mecom_gateway_stripe_cron_auto_sync' );
    wp_clear_scheduled_hook('mecom_gateway_stripe_daily');
    wp_clear_scheduled_hook('mecom_gateway_stripe_rotation');
}

function updateFeeNetOrderStripe($charge, $order)
{
    if (isset($charge->balance_transaction) && is_object($charge->balance_transaction)) {
        $display_order_currency = MECOM_STRIPE_FEE_DISPLAY_ORDER_CURRENCY;
...
function action_stripe_wp_head()
{
    if (is_checkout()) {
        ...
        if (isset($gateways['mecom_stripe']->enabled) && $gateways['mecom_stripe']->enabled == 'yes') {
            findAndSetNextProxy();
            echo '<link class="cs_stripe_element" rel="preload" href="' . WC()->session->get('mecom-stripe-proxy-active-url') . '?mecom-stripe-pe-v2-get-payment-form=1" as="document">';
        }
        if (isset($gateways['mecom_stripe']->enabled) && $gateways['mecom_stripe']->payment_mode == MECOM_STRIPE_PAYMENT_MODE_HOSTED) {
        ...
        if ($payForOrderObj instanceof WC_Order) {
            ...
            if (!empty($proxyUrl)) {
                ...
                if (!empty($storedCsOrderKey)) {
                    WC()->session->set('mecom-stripe-processing-order-key', $storedCsOrderKey);
                }
            } else {
                ...
                WC()->session->set('mecom-stripe-processing-order-key', $csOrderKey);
            ...
            if (!empty($proxyUrl)) {
                ...
                WC()->session->set('mecom-stripe-proxy-active-id', null);
                WC()->session->set('mecom-stripe-proxy-active-url', $proxyUrl);
                return;
            }
        }
    }
    ...
    if (!empty($orderIdProcessing) ) {
        ...
        if ($isEnableEndpointMode) {
            ...
            if ($orderProcessing instanceof WC_Order) {
                ...
                $proxyProcessing = findActivatedProxyDataByIdStripe(get_option(Opt_Mecom_Stripe_Proxies, []), $proxyProcessingId);
            }
        }
        if (isset($proxyProcessing)) {
            csStripeDebugLog($proxyProcessing, '$proxyProcessing');
            WC()->session->set('mecom-stripe-proxy-active-id', $proxyProcessing['id']);
            WC()->session->set('mecom-stripe-proxy-active-url', $proxyProcessing['url']);
            return;
        }
    }
    if ($isEnableEndpointMode) {
        ...
        if (isEnabledAmountRotationStripe()) {
            ...
            $nextProxy = get_option(Opt_Mecom_Stripe_Activated_Proxy, null);
        }
    }
    
    if (empty($nextProxy)) {
        WC()->session->set('mecom-stripe-proxy-active-id', null);
        WC()->session->set('mecom-stripe-proxy-active-url', null);
        return;
    }
    WC()->session->set('mecom-stripe-proxy-active-id', $nextProxy['id']);
    WC()->session->set('mecom-stripe-proxy-active-url', $nextProxy['url']);
}
...
function cs_add_order_note()
{
    ...
    $response = wp_remote_get(WC()->session->get('mecom-stripe-proxy-active-url') . '?' . csStripeBuildQuery([
            'mecom-stripe-pe-v2-get-payment-intent' => uniqid(),
            'payment_intent_id' => csStripeGetTransactionId($order),
            'merchant_site' => get_home_url(),
        ]), [
    ...
    if ($decideUpdateOrderFail) {
        ...
        $order->update_status('failed');
        $activeProxyId = WC()->session->get('mecom-stripe-proxy-active-id');
        $activatedProxy = findActivatedProxyDataByIdStripe(get_option(Opt_Mecom_Stripe_Proxies, []), $activeProxyId);
        wp_remote_post($activatedProxy['url'] . '?' . csStripeBuildQuery([
            'mecom-stripe-pe-v2-add-order-detail' => uniqid(),
            'transaction_id' => csStripeGetTransactionId($order),
...
function mecom_stripe_generate_input_order()
{
    ...
    <div id="mecom_stripe_pay_for_order_page"></div>
    <?php
}
...
function stripeGetChargeStatusByLoop($nextProxyUrl, $nextProxyId) {
    ...
    while (true) {
        ...
        if ($loopAccountStatus === 'active') {
            ...
            stripeMoveToUnusedProxyIds([WC()->session->get('mecom-stripe-proxy-active-id')]);
            csStripeSendMailShieldDie(WC()->session->get('mecom-stripe-proxy-active-url'));
            findAndSetNextProxy();
        } else if($loopRetry >= 3) {
            findAndSetNextProxy();
            return false;
...
function stripeGetShieldUrl()
{
    $loopCheckShield = 0;
    $hasShield = false;
    if (empty(WC()->session->get('mecom-stripe-proxy-active-id'))) {
        findAndSetNextProxy();
    }
    $firstProxyId = WC()->session->get('mecom-stripe-proxy-active-id');
    while (true) {
        $nextProxyUrl = WC()->session->get('mecom-stripe-proxy-active-url');
        $nextProxyId = WC()->session->get('mecom-stripe-proxy-active-id');
