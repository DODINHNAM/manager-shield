...
class MEcom_Stripe_Paygate_Option
{
    public function __construct()
    {
        add_action('admin_menu', [$this, 'add_mecom_stripe_paygate_menu']);
        add_action('wp_ajax_mecom_gateway_stripe_action', [$this, 'mecom_gateway_stripe_action']);
    ...
    function add_mecom_stripe_paygate_menu()
    {
        $mypage = add_menu_page('CardsShield Gateway Stripe Settings', 'CardsShield Stripe', 'manage_options', 'mecom-gateway-stripe', [$this, 'mecom_page_init']);
    ...
    function enqueue_scripts_front_end()
    {
        ...
        wp_register_style("mecom_settings_css", plugins_url('assets/css/settings.css', __FILE__), [], OPT_MECOM_STRIPE_VERSION);
        wp_enqueue_style('mecom_settings_css');


        // js
        wp_register_script("mecom_swal2", plugins_url('assets/js/sweetalert2.all.min.js', __FILE__), [], OPT_MECOM_STRIPE_VERSION);
        ...
        wp_register_script("mecom_stripe_settings", plugins_url('assets/js/settings.js', __FILE__), [], OPT_MECOM_STRIPE_VERSION);
        wp_enqueue_script("mecom_stripe_settings");


        // in JavaScript, object properties are accessed as ajax_object.ajax_url, ajax_object.we_value
        wp_localize_script('mecom_stripe_settings', 'ajax_object', ['ajax_url' => admin_url('admin-ajax.php'), 'we_value' => 1234]);
    }
    
    function cs_stripe_admin_order_list_top_bar_button( $type, $which ) {
        if ( 'shop_order' === $type && 'top' === $which ) {
            wp_register_style("mecom_woo_css", plugins_url('assets/css/woo_styles.css', __FILE__), [], OPT_MECOM_STRIPE_VERSION);
            wp_enqueue_style('mecom_woo_css');
    
            wp_register_script("mecom_stripe_woo_scripts", plugins_url('assets/js/woo_scripts.js', __FILE__), [], OPT_MECOM_STRIPE_VERSION);
            wp_enqueue_script("mecom_stripe_woo_scripts");
            wp_localize_script('mecom_stripe_woo_scripts', 'cs_ajax_object', [ 'ajax_url' => admin_url('admin-ajax.php'), 'we_value' => 1234 ] );
    ...
    function filter_orders_by_sync_status_query( $vars ) {
        ...
        if ( 'shop_order' === $typenow && isset( $_GET['_shop_stripe_order_sync_status'] ) && '' != $_GET['_shop_stripe_order_sync_status'] ) {
            $vars['meta_query'][] = array(
                'key'       => '_mecom_stripe_sync_tracking_info',
        ...
        if ( ! empty( $query_vars[ METAKEY_STRIPE_SYNC_TRACKING_INFO ] ) ) {
            ...
            $metaQuery = [
                ...
                [
                    'relation' => 'OR',
                    [
                        'key'   => METAKEY_MECOM_STRIPE_CAPTURED,
                        'compare' => 'NOT EXISTS'
                    ],
                    [
                        'key'   => METAKEY_MECOM_STRIPE_CAPTURED,
    ...
    function mecom_gateway_stripe_action()
    {
    ...
    function changeConnectionMode() {
        ...
        $isSuccess1 = update_option(OPT_MECOM_STRIPE_CONNECTION_MODE, $connectionMode, true);
    ...
    {
        $isSuccess = update_option(OPT_MECOM_STRIPE_ROTATION_METHOD, $_POST['rotationMethod'], true);
        echo json_encode([
            'success' => $isSuccess
        ]);
    }
    ...
    function activateProxy()
    {
        ...
        foreach ($proxies as $proxy) {
            if ($proxy["id"] == $proxyID) {
                // Active
                update_option(Opt_Mecom_Stripe_Activated_Proxy, $proxy, true);
    ...
    function deleteProxy()
    {
        ...
        $proxies = get_option(OPT_MECOM_STRIPE_UNUSED_PROXIES, []);
        foreach ($proxies as $key => $proxy) {
            if (in_array($proxy['id'], $deleteProxyIds)) {
                unset($proxies[$key]);
            }
        }
        $isSuccess = update_option(OPT_MECOM_STRIPE_UNUSED_PROXIES, array_values($proxies), true);
        echo json_encode([
            'success' => $isSuccess
        ]);
    }
    ...
    function addNewProxy()
    {
        ...
        if ($rotationMethod === MECOM_STRIPE_BY_TIME) {
            ...
            $proxy['amount'] = 0;
        } else if ($rotationMethod === MECOM_STRIPE_BY_AMOUNT) {
            $proxy['timestamp'] = 0;
            $proxy['amount'] = $rotationValue;
        }
        $proxies[] = $proxy;
        // Save
        $isSuccess = update_option(Opt_Mecom_Stripe_Proxies, $proxies, true);
        ...
        if (empty($activatedProxy)) {
            ...
            update_option(OPT_MECOM_STRIPE_CURRENT_ROTATION_VALUE, time(), true);
            update_option(OPT_MECOM_STRIPE_ROTATION_METHOD, $rotationMethod, true);
        }
    ...
    function saveProxies()
    {
        ...
        foreach ($proxies as $key => $proxy) {
            ...
            $proxies[$key]['timestamp'] = $rotationMethod === MECOM_STRIPE_BY_TIME ? $newProxies[$key]['rotationValue'] : $proxies[$key]['timestamp'];
            $proxies[$key]['amount'] = $rotationMethod === MECOM_STRIPE_BY_AMOUNT ? $newProxies[$key]['rotationValue'] : $proxies[$key]['amount'];

            // Update activated proxy
            if (isset($activatedProxy) && $activatedProxy['id'] === $proxy['id']) {
                update_option(Opt_Mecom_Stripe_Activated_Proxy, $proxies[$key], true);
            }
        }
        update_option(Opt_Mecom_Stripe_Proxies, $proxies, true);
        echo json_encode([
            "success" => true
        ]);
    }
    ...
    function moveBackProxies()
    {
        ...
        $isSuccess1 = update_option(Opt_Mecom_Stripe_Proxies, $proxies, true);
        $isSuccess2 = update_option(OPT_MECOM_STRIPE_UNUSED_PROXIES, array_values($unusedProxies), true);
    ...
    switch ( $trackingSyncPlugin ) {
        case OPT_CS_STRIPE_TRACKING_SYNC_PLUGIN_ADVANCED_SHIPMENT_TRACKING:
            ...
            ", METAKEY_STRIPE_SYNC_TRACKING_INFO , METAKEY_MECOM_STRIPE_CAPTURED, OPT_CS_STRIPE_NOT_SYNCED) );
    ...
    {
        $connectionMode = get_option(OPT_MECOM_STRIPE_CONNECTION_MODE, OPT_CS_STRIPE_CONNECTION_MODE_SHIELD_DOMAINS);
        $rotationMethod = get_option(OPT_MECOM_STRIPE_ROTATION_METHOD, MECOM_STRIPE_BY_TIME);
        if (empty($rotationMethod)) {
            $rotationMethod = MECOM_STRIPE_BY_TIME;
            update_option(OPT_MECOM_STRIPE_ROTATION_METHOD, MECOM_STRIPE_BY_TIME, true);
        }
        ...
        <style>
            .by-time {
            <?= $rotationMethod !== MECOM_STRIPE_BY_TIME ? 'display: none' : '' ?>;
            }

            .by-amount {
            <?= $rotationMethod !== MECOM_STRIPE_BY_AMOUNT ? 'display: none' : '' ?>;
            }
        </style>
        <br/>
        <div class="container">
            ...
            <div class="row">
                <div class="col-sm">
                    <div class="form-group rotation-method-wrapper">
                        ...
                        <div>Connect with shields by endpoint token. please go to <a
                                    href="https://manager.cardsshield.com">manager.cardsshield.com</a> to setup an
                            endpoint token.
                        </div>
                    </div>
                </div>
            </div>
            <div id="connection_mode_shield_domains_area"
                        ...
                        <div class="form-group form-inline rotation-method-wrapper">
                            ...
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="rotationMethod" id="rotationByTime"
                                       value="by_time" <?= $rotationMethod === MECOM_STRIPE_BY_TIME ? 'checked' : '' ?> >
                            ...
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="rotationMethod" id="rotationByAmount"
                                       value="by_amount" <?= $rotationMethod === MECOM_STRIPE_BY_AMOUNT ? 'checked' : '' ?> >
                ...
                <div class="row">
                    <div class="col-sm">
                        <table class="table table-proxy table-hover table-borderless">
                            ...
                            <?php
                            foreach ($proxies as $proxy) {
                                $proxy["rotationValue"] = $rotationMethod === MECOM_STRIPE_BY_TIME ? $proxy["timestamp"] : $proxy["amount"];
...
new MEcom_Stripe_Paygate_Option();
