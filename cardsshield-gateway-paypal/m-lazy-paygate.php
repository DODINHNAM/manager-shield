        ...
        if ( is_checkout() ) {
            ...
            if ( isset( $gateways['lazy_paypal']->enabled ) && $gateways['lazy_paypal']->enabled == 'yes' ) {
                 ...
                 if ($nextProxyUrl && $ppBtnSetting === OPT_CS_PAYPAL_SETTING_CHECKOUT) {
                    ...
                    $proxyFullUrl = $nextProxyUrl . '/checkout?session=' .csPpGenerateRandomString(35). '&checkout=yes&intent=' . $intentIframe . '&currency=' . get_woocommerce_currency() . '&Origin=' . get_site_url();
    ...
    function lazy_paypal_add_checkout_button_at_product_page() {
        ...
        if ( isset( $gateways['lazy_paypal']->enabled ) && $gateways['lazy_paypal']->enabled == 'yes' ) {
            ...
            <div id="lazy-paypal-product-page-has-variations" data-value="<?= $isProdHasVariations ? 'yes' : 'no' ?>"></div>
             ...
             if ($nextProxyUrl && $ppBtnSetting === OPT_CS_PAYPAL_SETTING_CHECKOUT) {
                $intentIframe = strtolower( $ppGatewayObj->get_option('intent'));
                $proxyFullUrl = $nextProxyUrl . '/checkout?session=' .csPpGenerateRandomString(35). '&checkout=yes&express_checkout=1&intent=' . $intentIframe . '&currency=' . get_woocommerce_currency() . '&Origin=' . get_site_url();
                if ($ppGatewayObj->get_option('disable_credit_card') == 'yes') {
                    $proxyFullUrl .= '&disable_credit_card=1';
                }
